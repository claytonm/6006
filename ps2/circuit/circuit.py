...
class PriorityQueue:
    ...
    Problem set requires rewriting this class to implement a min-heap, rather
    than the array-based, priority queue.
    """
    ...
    def __init__(self):
        ...
        self.min_index = 0
        self.length = 0
        # maxTransition is a sentinel value,
        # the largest possible object of time Transition
        self.maxTransition = Transition(Gate('maxGate',
                                             GateType('maxGate',
                                                      TruthTable('maxGate', [0,1]),
                                                      1)),
                                        0,
                                        float('inf'))
    ...
    def append(self, key):
        ...
        if len(self) > len(self.queue):
            self.queue.append(self.maxTransition)
        else:
            self.queue[len(self) - 1] = self.maxTransition
